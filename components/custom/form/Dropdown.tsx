...
/* --- types --- */
export type DropdownItemProps = { label: string; value: string | number; key?: string | number };
...
function DropdownBase(props: DropdownProps) {
  ...
  const items = useMemo<DropdownItemProps[]>(() => {
    ...
    React.Children.forEach(children, (child) => {
      ...
      if (p.label !== undefined || p.value !== undefined) {
        out.push({ label: p.label ?? String(p.value ?? ""), value: p.value, key: p.key ?? p.value });
      }
    });
    return out;
  }, [children]);
  ...
  const floatStart = enableFloating && ((selectedValue !== null && selectedValue !== "" && selectedValue !== undefined) || items.length > 0);
  const floatAnim = useRef(new Animated.Value(floatStart ? 1 : 0)).current;

  useEffect(() => {
    if (!enableFloating) return;
    const shouldFloat = Boolean((selectedValue !== null && selectedValue !== "" && selectedValue !== undefined) || visible);
    Animated.timing(floatAnim, { toValue: shouldFloat ? 1 : 0, duration: 160, easing: Easing.inOut(Easing.ease), useNativeDriver: true }).start();
  }, [enableFloating, selectedValue, visible, floatAnim]);

  const labelTranslateY = floatAnim.interpolate({ inputRange: [0, 1], outputRange: [6, -18] });
  const labelScale = floatAnim.interpolate({ inputRange: [0, 1], outputRange: [1, 0.85] });
    ...
    <View style={{ width: "100%" }}>
      {Platform.OS === "android" ? (
        <View style={styles.inlineList}>
          {items.map((it) => (
            <TouchableOpacity key={String(it.key ?? it.value)} onPress={() => onValueChange?.(it.value)} style={styles.inlineItem}>
              <Text style={[styles.itemText, it.value === selectedValue ? { fontWeight: "700" } : undefined]}>{it.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      ) : (
        <Text style={styles.placeholderText}>Inline selection not available on this platform. Use useModal={true}.</Text>
      )}
    </View>
  );
  ...
  return (
    <View style={{ width: "100%", position: "relative", overflow: "visible" }} testID={testID}>
      ...
      {enableFloating && label ? (
        <Animated.View pointerEvents="none" style={{ position: "absolute", left: 12, top: 12, transform: [{ translateY: labelTranslateY }, { scale: labelScale }], backgroundColor: "#fff", paddingHorizontal: 4, zIndex: 10 }}>
      ...
      {/* Trigger */}
      <TouchableOpacity activeOpacity={0.9} onPress={open} disabled={disabled} style={[styles.control, enableFloating && label ? { paddingTop: 20 } : undefined]}>
        <Text style={[styles.selectedText, selectedValue === null || selectedValue === undefined || selectedValue === "" ? styles.placeholderText : undefined]} numberOfLines={1}>
          {selectedLabel}
        </Text>
      ...
      {/* helper / error */}
      {error ? <Text style={[styles.helperText, { color: "#dc2626" }]}>{error}</Text> : helper ? <Text style={styles.helperText}>{helper}</Text> : null}
            ...
            <View style={modalStyles.card}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity onPress={() => setTempValue(item.value)} style={[modalStyles.item, sel && modalStyles.itemSelected]}>
                      <Text style={[modalStyles.itemText, sel && { fontWeight: "700" }]}>{item.label}</Text>
                    </TouchableOpacity>
                  );
                }}
...
const Dropdown = DropdownBase as typeof DropdownBase & { Item: typeof DropdownItem };
...
const styles = StyleSheet.create({
  ...
  // inline fallback
  inlineList: { borderWidth: 1, borderColor: "#d1d5db", borderRadius: 8, overflow: "hidden", backgroundColor: "#fff" },
...
const modalStyles = StyleSheet.create({
  ...
  card: { backgroundColor: "white", borderRadius: 12, maxHeight: "80%", overflow: "hidden" },
  header: { flexDirection: "row", alignItems: "center", padding: 12, borderBottomWidth: 1, borderColor: "#eee", justifyContent: "space-between" },
